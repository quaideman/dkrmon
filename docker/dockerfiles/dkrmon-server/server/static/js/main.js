...
function buildTable(container,data,columns,pageLimit){
  ...
  function containers(container){
    ...
    // ID
    table.find('.cell[data-page="'+pageCount+'"][data-row="'+rowCount+'"][data-column="Id"] span').text(container['Id']).attr('title',container['Id']);
    // Command
    var command = "";
    $.each(container['Config']['Cmd'],function(key,val){
      command += val +' '
    })
    table.find('.cell[data-page="'+pageCount+'"][data-row="'+rowCount+'"][data-column="Command"] span').text(command).attr('title',command);
    // Image
    ...
    if (container['State']['Health']) {
      table.find('.cell[data-page="'+pageCount+'"][data-row="'+rowCount+'"][data-column="Health"] span').text(container['State']['Health']['Status']).attr('title',container['State']['Health']['Status']);
    } else {
      table.find('.cell[data-page="'+pageCount+'"][data-row="'+rowCount+'"][data-column="Health"] span').text('NA').attr('title','NA');
    }
    // Memory
    if (container['Memory']) {
  ...
  $.each(data,function(i,v){
    ...
    $.each(columns,function(i,column){
      var cell = '<div class="cell" data-page="'+pageCount+'" data-row="'+rowCount+'" data-column="'+column+'"><span></span></div>'
      table.append(cell)
    })
    // Iterarte over object attributes
    // $.each(v,function(key,value){
    //   if (table.attr('data-table') == 'containers') {containers(key,value)}
    // })
    containers(v)
  ...
  container.find('[data-fn="nextPage"]').off().click(function(){
    ...
    if ( table.attr('data-filter') ) {
      ...
      if ( table.find('.cell[data-filterPage="'+nextFilterPage+'"]').length > 0 ) {
        ...
        table.find('.cell[data-filterPage="'+nextFilterPage+'"]').css('display','flex');;
      }
    } else {
      ...
      if ( table.find('.cell[data-page="'+nextPage+'"]').length > 0 ) {
        ...
        table.find('.cell[data-page="'+nextPage+'"]').css('display','flex');;
      }
    }
      ...
      case 'pruneImages':
        ...
        if (data['resultData']['ImagesDeleted'] == null) {
          ...
          $.each(data['resultData']['ImagesDeleted'],function(){
            var html = '<div><span class="icon success material-icons">'+icon+'</span><span class="message">'+this['Deleted']+'</span></div>'
            element.append(html);
          })
        }
...
var mod = {
  ...
  containers : {
    ...
    responseOLD: function(thisHost,responseData) {
    ...
    load: function(thisHost) {
      ...
      thisMod.find('[data-fn="filterTable"]').keydown(function(){
        var thisEl = $(this);
        thisMod.find('.header-controls .badge').attr('data-active','false')
        clearTimeout(filterDelay);
        filterDelay = setTimeout(function(){
          var query = thisEl.val();
      ...
      // Attach events to count badges
      thisMod.find('.header-controls .badge[data-fn="badgeFilter"]').click(function(){
        $(this).siblings().attr('data-active','false');
        thisMod.find('[data-fn="filterTable"]').val('');
        if ( $(this).attr('data-active') == 'true') {
          $(this).attr('data-active','false')
          var value = ''
          filterTable(value,thisMod);
        } else {
          $(this).attr('data-active','true')
          var value = $(this).children('[data-value]').attr('data-value');
          filterTable(value,thisMod);
        }
